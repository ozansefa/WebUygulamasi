<?php
    ...
    $admin_passwd = $_POST['admin_passwd'] ?? null;

    if($_POST){
        $con = new PDO('mysql:host=localhost;dbname=site-veritabani;charset=utf8', 'root', '');
        $sorgu = $con -> prepare('SELECT * FROM admin WHERE admin_email=? AND admin_passwd=?');
        $sorgu -> execute([$admin_email, $admin_passwd]);
        $icerik = $sorgu -> fetch(PDO::FETCH_ASSOC);
        if($icerik){
            $_SESSION['KULLANICI_ADI'] = $admin_email;
            Header('Location:icerik.php');
        }
        else{
			...
			<div class="wrap-login100">
				<form action="index.php" method="POST" class="login100-form validate-form">
					...
					<div class="wrap-input100 validate-input" data-validate="Enter password">
						<input class="input100" type="password" name="admin_passwd" placeholder="Password">
						<span class="focus-input100" data-placeholder="&#xf191;"></span>
					</div>
